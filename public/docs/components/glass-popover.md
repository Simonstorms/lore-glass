...
| `sideOffset` | `number` | `10` | Gap between trigger and panel in px. |
| `tint` | `number` | `0.55` | Glass tint of the panel from 0 (clear) to 1 (opaque). |
...
| Prop | Type | Default | Description |
| --- | --- | --- | --- |
| `label` | `string` | `required` | Row label, also used for keyboard typeahead. |
| `sublabel` | `string` | `undefined` | Smaller secondary line under the label. |
| `icon` | `ReactNode` | `undefined` | Leading icon slot. |
| `destructive` | `boolean` | `false` | Tints the row red for destructive actions. |
| `onClick` | `(event) => void` | `undefined` | Action handler. The menu closes on click by default. |
